...
def get_video_formats():
    ...
    for format_name in folder_paths.get_filename_list("VHS_video_formats"):
        ...
        video_format_path = folder_paths.get_full_path(
            "VHS_video_formats", format_name + ".json"
        )
    ...
    for w in gen_format_widgets(video_format):
        if len(w[0]) > 2 and "default" in w[0][2]:
            ...
            if type(w[0][1]) is list:
                ...
                default = {"BOOLEAN": False, "INT": 0, "FLOAT": 0, "STRING": ""}[
                    w[0][1]
                ]
...
def ffmpeg_process(args, video_format, video_metadata, file_path, env):
    ...
    if video_format.get("save_metadata", "False") != "False":
        ...
        with subprocess.Popen(
            m_args + [file_path], stderr=subprocess.PIPE, stdin=subprocess.PIPE, env=env
        ) as proc:
            try:
...
class VideoCombine:
    ...
    def INPUT_TYPES(s):
        ...
        return {
            ...
            "optional": {"audio": ("VHS_AUDIO",), "meta_batch": ("VHS_BatchManager",)},
            "hidden": {
                "prompt": "PROMPT",
    ...
    def combine_video(
        ...
        if format_type == "image":
            if meta_batch is not None:
                raise Exception(
                    "Pillow('image/') formats are not compatible with batched output"
                )
            image_kwargs = {}
            if format_ext == "gif":
            ...
            if kwargs is None:
                ...
                if len(missing) > 0:
                    logger.warn(
                        "Extra format values were not provided, the following defaults will be used: "
                        + str(kwargs)
            ...
            if loop_count > 0:
                loop_args = [
                    "-vf",
                    "loop=loop=" + str(loop_count) + ":size=" + str(len(images)),
                ]
            else:
                loop_args = []
            if pingpong:
            ...
            if "gifski_pass" in video_format:
                ...
                try:
                    res = subprocess.run(
                        gifski_args, env=env, check=True, capture_output=True
                    )
                except subprocess.CalledProcessError as e:
                    raise Exception(
                        "An error occured in the gifski subprocess:\n"
                ...
                output_file_with_audio = (
                    f"{filename}_{counter:05}-audio.{video_format['extension']}"
                )
                output_file_with_audio_path = os.path.join(
                    full_output_folder, output_file_with_audio
                )
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
                ...
                "start_time": (
                    "FLOAT",
                    {"default": 0, "min": 0, "max": 10000000, "step": 0.01},
                ),
                "duration": (
                    "FLOAT",
                    {"default": 0, "min": 0, "max": 10000000, "step": 0.01},
...
class PruneOutputs:
    ...
    def prune_outputs(self, filenames, options):
        ...
        for file in delete_list:
            ...
            if (os.path.commonpath([output_dirs[0], file]) != output_dirs[0]) and (
                os.path.commonpath([output_dirs[1], file]) != output_dirs[1]
            ):
...
class ProcessOutputs:
    ...
    def process_outputs(self, filenames, options):
        if len(filenames[1]) == 0:
            return ()
        assert len(filenames[1]) <= 3 and len(filenames[1]) >= 2
        delete_list = []
...
NODE_CLASS_MAPPINGS = {
    ...
    "VHS_ProcessOutputs": ProcessOutputs,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "VHS_ProcessOutputs": "Process Outputs 🎥🅥🅗🅢",
